...
def set_gpu_usage(gpu_memory_fraction):
    pass # because it was changed in TF2
...
def classify_image(img, model, multi_label_classification, target_size=(224, 224, 3), multi_thresh=0.5):
    # resize
    img = img / 255.
    ...
    if multi_label_classification:
        predictions[predictions >= multi_thresh] = 1
        predictions[predictions < multi_thresh] = 0
    else:
...
def get_multilabel_evaluation_metrics(pred, labels, class_names, thresh_range=(0.01, 0.99), image_names=None, save_path = None):
    current_auroc = []
    ...
    exact_accuracy,best_exact_thresh = get_best_exact_match(pred,labels,thresh_range)
